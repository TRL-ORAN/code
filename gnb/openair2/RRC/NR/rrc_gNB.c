...
const neighbour_cell_configuration_t *get_neighbour_config(int serving_cell_nr_cellid)
{
  ...
  LOG_W(NR_RRC, "get_neighbour_config: Called with serving_cell_nr_cellid = %d\n", serving_cell_nr_cellid); // Use %ld if it's long

  seq_arr_t *neighbour_cell_configuration = rrc->neighbour_cell_configuration;
  if (!neighbour_cell_configuration){
    LOG_W(NR_RRC, "!!!Neighbour cell configuration is NULL.\n"); 
    return NULL;
  }
  LOG_W(NR_RRC, "???get_neighbour_config: rrc->neighbour_cell_configuration size = %d\n", neighbour_cell_configuration->size);
  ...
  for (int cellIdx = 0; cellIdx < neighbour_cell_configuration->size; cellIdx++) {
        neighbour_cell_configuration_t *current_config_in_list =
            (neighbour_cell_configuration_t *)seq_arr_at(neighbour_cell_configuration, cellIdx);
        if (current_config_in_list) {
            LOG_W(NR_RRC, "get_neighbour_config: Checking list entry %d, nr_cell_id = %d\n", // Use %ld if long
                  cellIdx, current_config_in_list->nr_cell_id);
            if (current_config_in_list->nr_cell_id == serving_cell_nr_cellid) {
                LOG_W(NR_RRC, "get_neighbour_config: Match found for %d!\n", serving_cell_nr_cellid); // Use %ld if long
                return current_config_in_list;
            }
        } else {
            LOG_W(NR_RRC, "get_neighbour_config: Entry %d in rrc->neighbour_cell_configuration is NULL\n", cellIdx);
        }
    }
    LOG_W(NR_RRC, "get_neighbour_config: No match found for %d after iterating through all entries.\n", serving_cell_nr_cellid); // Use %ld
...
void openair_rrc_gNB_configuration(gNB_RRC_INST *rrc, gNB_RrcConfigurationReq *configuration)
{
  ...
  // *******************************************************************
  if (rrc->neighbour_cell_configuration != NULL) {
    LOG_I(NR_RRC, "Neighbour cell configuration loaded. Number of serving cell entries: %d\n",
          rrc->neighbour_cell_configuration->size);

    for (int i = 0; i < rrc->neighbour_cell_configuration->size; i++) {
      neighbour_cell_configuration_t *s_cfg =
          (neighbour_cell_configuration_t *)seq_arr_at(rrc->neighbour_cell_configuration, i);
      if (s_cfg) {
        LOG_I(NR_RRC, "  Serving Cell ID: %d has %d configured neighbours.\n",
              s_cfg->nr_cell_id,
              s_cfg->neighbour_cells ? s_cfg->neighbour_cells->size : 0);
        
        if (s_cfg->neighbour_cells && s_cfg->neighbour_cells->size > 0) {
            for (int j = 0; j < s_cfg->neighbour_cells->size && j < 3; ++j) { 
                 nr_neighbour_gnb_configuration_t *n_cfg =
                     (nr_neighbour_gnb_configuration_t *)seq_arr_at(s_cfg->neighbour_cells, j);
                 if (n_cfg) {
                     LOG_I(NR_RRC, "    -> Neighbour PCI: %d\n", n_cfg->physicalCellId);
                 }
            }
        }
      }
    ...
    LOG_W(NR_RRC, "Neighbour cell configuration (rrc->neighbour_cell_configuration) is NULL after RRC configuration.\n");
  }
  // *******************************************************************
  // *******************************************************************
}
                                             ...
                                             bool reestablish)
{
  ...
  LOG_W(NR_RRC, "cell_info: nr_cellid=%ld, nr_pci=%d\n", cell_info->nr_cellid, cell_info->nr_pci); // Print cell_info details

  NR_MeasConfig_t *measconfig = NULL;

  if (du->mtc != NULL) {
    LOG_W(NR_RRC, "du->mtc is not NULL. Proceeding with measurement configuration.\n");
    ...
    if (neighbour_config){
      ...
      LOG_W(NR_RRC, "MeasConfig: Found neighbour_config for serving cell %d, num_neighbours: %d\n",
          cell_info->nr_cellid, neighbour_cells ? neighbour_cells->size : 0); 
    } else {
      LOG_W(NR_RRC, "########MeasConfig: No neighbour_config found for serving cell %d.\n", cell_info->nr_cellid); 
    }
    ...
    case NR_EventTriggerConfig__eventId_PR_eventA3: {    
      ...
      for (int neigh_meas_idx = 0; neigh_meas_idx < measResultListNR->list.count; neigh_meas_idx++) {
        ...
        if (!neigh_cell && neighbour) {
          ...
          LOG_W(NR_RRC, "HO LOG: Potential F1 HO to neighbour PCI %d (configured).\n", neighbourCellId); 
...
static void rrc_gNB_process_MeasurementReport(gNB_RRC_UE_t *UE, NR_MeasurementReport_t *measurementReport)
{
  ...
  //const NR_MeasResults_t *measResults = &measurementReport_IEs->measResults;
  const NR_MeasResults_t *measResults = UE->measResults;

  // ************************************************************************************
  // ***** *****
  // ************************************************************************************
  if (!measResults) {
    LOG_W(NR_RRC, "Condition fail: measResults is NULL\n");
} else if (!measResults->measResultNeighCells) {
    LOG_W(NR_RRC, "Condition fail: measResults->measResultNeighCells is NULL\n");
} else if (measResults->measResultNeighCells->present != NR_MeasResults__measResultNeighCells_PR_measResultListNR) {
    LOG_W(NR_RRC, "Condition fail: measResultNeighCells->present is not NR_MeasResults__measResultNeighCells_PR_measResultListNR (actual value: %d)\n", measResults->measResultNeighCells->present);
} else if (!measResults->measResultNeighCells->choice.measResultListNR) {
    LOG_W(NR_RRC, "Condition fail: measResults->measResultNeighCells->choice.measResultListNR is NULL\n");
} else if (!(measResults->measResultNeighCells->choice.measResultListNR->list.count > 0)) {
    LOG_W(NR_RRC, "Condition fail: measResultNeighCells->choice.measResultListNR->list.count is %d (not > 0)\n", measResults->measResultNeighCells->choice.measResultListNR->list.count);
} else {
    LOG_I(NR_RRC, "UE %u (RNTI %04x) REPORTED NEIGHBOUR CELL INFORMATION ... \n", UE->rrc_ue_id, UE->rnti);
}
  // ************************************************************************************
  // *********
  // ************************************************************************************
  ...
  NR_ReportConfigToAddMod__reportConfig *report_config = NULL;
...
static void print_rrc_meas(FILE *f, const NR_MeasResults_t *measresults)
{
  ...
  if (measresultnr->physCellId)
    ...
    LOG_W(NR_RRC, "//////servingCellId %ld MeasResultNR for phyCellId %ld:\n      resultSSB:", measresultservmo->servCellId,
    *measresultnr->physCellId);
  if (mqr != NULL) {
    ...
    LOG_W(NR_RRC, "RSRP %ld dBm RSRQ %.1f dB SINR %.1f dB\n", rrsrp, rrsrq, rsinr);
  } else {
  ...
  if (measresults->measResultNeighCells
    ...
    for (int i = 0; i < meas_neigh->list.count; ++i) {
      ...
      if (measresultneigh->physCellId)
        ...
        LOG_E(NR_RRC, "//////neighboring cell for phyCellId %ld:\n      resultSSB:", *measresultneigh->physCellId);
      if (mqr != NULL) {
        ...
        fprintf(f, "RSRP %ld dBm RSRQ %.1f dB SINR %.1f dB\n", rrsrp, rrsrq, rsinr);
        LOG_E(NR_RRC, "RSRP %ld dBm RSRQ %.1f dB SINR %.1f dB\n", rrsrp, rrsrq, rsinr);
      } else {
        fprintf(f, "NOT PROVIDED\n");
        LOG_E(NR_RRC, "NOT PROVIDED\n");
      }
    }
  }
}
