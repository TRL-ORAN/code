...
void nr_HO_F1_trigger_telnet(gNB_RRC_INST *rrc, uint32_t rrc_ue_id)
{
  ...
  // LOG_E(NR_RRC, "############################## target_du_id %d gNB_DU_id%u\n", target_du->setup_req->gNB_DU_id, source_du->setup_req->gNB_DU_id);
...
void nr_HO_F1_trigger_to_target_du(gNB_RRC_INST *rrc, uint32_t rrc_ue_id, sctp_assoc_t target_du_id)
{
  rrc_gNB_ue_context_t *ue_context_p = rrc_gNB_get_ue_context(rrc, rrc_ue_id);
  if (ue_context_p == NULL) {
    LOG_E(NR_RRC, "cannot find UE context for UE ID %d\n", rrc_ue_id);
  ...
  nr_rrc_du_container_t *target_du = find_du_by_assoc_id(rrc, target_du_id);

  // LOG_E(NR_RRC, "############################## target_du_id %d gNB_DU_id%u\n", target_du->setup_req->gNB_DU_id, source_du->setup_req->gNB_DU_id);
  
  if (target_du == NULL) {
    LOG_E(NR_RRC, "No DU found with assoc_id %d. Aborting HO for UE %u\n", target_du_id, ue->rrc_ue_id);
    return;
  }
  if (source_du == target_du) {
    LOG_W(NR_RRC, "Source DU and Target DU are the same (assoc_id %d) for UE %u. Aborting HO.\n", target_du_id, ue->rrc_ue_id);
    return;
  }
  nr_rrc_trigger_f1_ho(rrc, ue, source_du, target_du);
}
...
void HO_UE_to_target_du_id(gNB_RRC_INST *rrc, uint32_t rrc_ue_id, uint64_t target_cell_id)
{
  rrc_gNB_ue_context_t *ue_context_p = rrc_gNB_get_ue_context(rrc, rrc_ue_id);
  if (ue_context_p == NULL) {
    LOG_E(NR_RRC, "cannot find UE context for UE ID %d\n", rrc_ue_id);
  ...
  nr_rrc_du_container_t *target_du = find_du_by_cell_id(rrc, target_cell_id);

  // LOG_E(NR_RRC, "############################## target_du_id %d gNB_DU_id%u\n", target_du->setup_req->gNB_DU_id, source_du->setup_req->gNB_DU_id);
  
  if (target_du == NULL) {
    LOG_E(NR_RRC, "No DU found with assoc_id %d. Aborting HO for UE %u\n", target_cell_id, ue->rrc_ue_id);
    return;
  }
  if (source_du == target_du) {
    LOG_W(NR_RRC, "Source DU and Target DU are the same (assoc_id %d) for UE %u. Aborting HO.\n", target_cell_id, ue->rrc_ue_id);
