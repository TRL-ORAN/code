...
//  assert(data->type == RAN_CONTROL_CTRL_V1_03 );
  ...
  printf("ctrl->hdr.frmt_1.ric_style_type = %d and ctrl->hdr.frmt_1.ctrl_act_id == %d", ctrl->hdr.frmt_1.ric_style_type, ctrl->hdr.frmt_1.ctrl_act_id );
  if(ctrl->hdr.frmt_1.ric_style_type == 1 && ctrl->hdr.frmt_1.ctrl_act_id == 2) {
    ...
    // }
  }  else if (ctrl->hdr.frmt_1.ric_style_type == 3 && ctrl->hdr.frmt_1.ctrl_act_id == Interference_PRB_quotal_7_6_3_1) {
      printf("####################################ctrl->hdr.frmt_1.ric_style_type == 2 && ctrl->hdr.frmt_1.ctrl_act_id == Interference_PRB_quotal_7_6_3_1");
      // // solve inter-cell interference  ///
      //  e2sm_rc_ctrl_msg_frmt_1_t const* msg = &ctrl->msg.frmt_1;
      //  seq_ran_param_t* RRM_Policy_Ratio_List = &msg->ran_param[0];
      // if (RRM_Policy_Ratio_List->ran_param_val.lst) {
      //   size_t ue_len = RRM_Policy_Ratio_List->ran_param_val.lst->sz_lst_ran_param;
      //   printf("ue_len = %d", ue_len);
      //   const int mod_id = 0;
      //   bool rc = schedule_interference_map(mod_id, ue_len, RRM_Policy_Ratio_List->ran_param_val.lst);
      //   if (!rc)
      //     LOG_E(NR_MAC, "failed schedule_interference_map\n");
      // } else {
      //   LOG_I(NR_MAC, "RRM_Policy_Ratio_List->ran_param_val.lst is NULL\n");
      // }
    }  else if (ctrl->hdr.frmt_1.ric_style_type == 3 && ctrl->hdr.frmt_1.ctrl_act_id == handover_PRB_quotal_7_6_3_1) {
      printf("####################################ctrl->hdr.frmt_1.ric_style_type == 2 && ctrl->hdr.frmt_1.ctrl_act_id == handover_PRB_quotal_7_6_3_1");
      // add handover and resource management   ///
       e2sm_rc_ctrl_msg_frmt_1_t const* msg = &ctrl->msg.frmt_1;
       seq_ran_param_t* RRM_Policy_Ratio_List = &msg->ran_param[0];
      if (RRM_Policy_Ratio_List->ran_param_val.lst) {
        size_t ue_len = RRM_Policy_Ratio_List->ran_param_val.lst->sz_lst_ran_param;
        printf("ue_len = %d", ue_len);
        const int mod_id = 0;
        bool rc = schedule_handover(mod_id, ue_len, RRM_Policy_Ratio_List->ran_param_val.lst);
        if (!rc)
          LOG_E(NR_MAC, "failed schedule_interference_map\n");
      } else {
        LOG_I(NR_MAC, "RRM_Policy_Ratio_List->ran_param_val.lst is NULL\n");
...
bool schedule_handover(int mod_id, size_t ue_len, ran_param_list_t* lst)
{
  ...
  for (size_t i = 0; i < ue_len; ++i) {
    ...
    assert(RRM_Policy_Ratio_Group->ran_param_struct.sz_ran_param_struct == num_map && "wrong RRM_Policy_Ratio_Group->ran_param_struct.sz_ran_param_struct");
