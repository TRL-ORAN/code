                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    //LOG_E(NR_MAC, "%f\n", UE->interference_map[0]);//
    //LOG_E(NR_MAC, "%f\n", UE->interference_map[2]);

    int count = 0;
    int total = 10;
    float max =  UE->interference_map[0];
    int max_id = 0;
    for (int i = 0; i < total; i++) {
        if (UE->interference_map[i] > 0) {
            count++;
        }
        if (UE->interference_map[i]>max) {
          max =  UE->interference_map[i];
          max_id = i * 10;
        }
    } 
    
    float map_proportion = (float)count / total;
  ...
  while (iterator->UE != NULL) {
    ...
    //printf("##########slbitmap  = %d", slbitmap);Add commentMore actions
    //LOG_E(NR_MAC, "UE->interference_map[j] %f\n", iterator->UE->interference_map[i]);

    int total = 5;
    float max =  iterator->UE->interference_map[0];
    int max_id = 0;
    for (int i = 0; i < total; i++) {
        if (iterator->UE->interference_map[i]>max) {
          max =  iterator->UE->interference_map[i];
          //LOG_E(NR_MAC, "UE->interference_map[j] %d max_id %f\n", max_id, iterator->UE->interference_map[i]);
          max_id = i * 10;
        }
    } 
